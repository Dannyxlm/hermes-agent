...
def push_skills(client: Optional[SyncClient] = None, *, skill_names: Optional[List[str]] = None,
    ...
    try:
        ...
        commit_hash = build_commit(root_hash, [], owner=owner,
                                   device=stable_device_id(), message=message, objects=objects)
        client.put_objects({commit_hash: objects.objects[commit_hash]})
        client.cas_ref(ref, None, commit_hash)
        result.update(head=commit_hash, pushed_objects=len(objects), recovered_stale_head=True)
