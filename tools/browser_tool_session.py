...
def _create_local_session(task_id: str, allow_real_profile: bool = True) -> Dict[str, str]:
    ...
    if allow_real_profile:
        ...
        if cdp_url:
            metadata = _real_profile._real_profile_metadata_for_cdp(cdp_url)
            info = _session_record("rp", _cdp._resolve_cdp_override(cdp_url), {
                "local": True, "real_profile": True,
                "real_profile_scope": metadata.get("key"),
                "real_profile_copy_root": metadata.get("copy_root"),
            })
...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if existing_session is not None and (existing_session.get("features") or {}).get("real_profile"):
        revoke = not _cloud._use_real_profile()
        if not revoke:
            scope, _error = _real_profile._real_profile_scope()
            revoke = not scope or scope.get("key") != (existing_session.get("features") or {}).get("real_profile_scope")
        if revoke:
            _real_profile._revoke_real_profile_for_current_home()
