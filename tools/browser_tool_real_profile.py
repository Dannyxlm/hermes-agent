...
def _terminate_real_profile_chrome(scope_key: Optional[str] = None, copy_root: Optional[str] = None) -> None:
    """Terminate only matching profile-copy processes; no scope means atexit cleanup."""
    from tools.browser_lightpanda import _terminate
    _bt = _origin()
    remaining = []
    while _bt._real_profile_chrome_procs:
        item = _bt._real_profile_chrome_procs.pop()
        proc_scope, proc_root, proc = item
        if ((scope_key is not None and proc_scope != scope_key)
                or (copy_root is not None and proc_root != copy_root)):
            remaining.append(item)
            continue
        _terminate(proc, what="real-profile chrome")
    _bt._real_profile_chrome_procs.extend(reversed(remaining))
...
def _real_profile_scope() -> tuple:
    ...
    return {
        "browser": browser,
        "copy_dir": copy_dir,
        "copy_root": os.path.realpath(str(_origin().get_hermes_home() / "browser-profile")),
        "key": key,
        "session_name": f"{_origin()._REAL_PROFILE_SESSION}-{key}",
...
def _real_profile_metadata_for_cdp(cdp_url: str) -> dict:
    with _origin()._real_profile_cdp_lock:
        for entry in _origin()._real_profile_cdp_cache.values():
...
def _revoke_real_profile_for_current_home() -> None:
    ...
    with _origin()._real_profile_cdp_lock:
        entries = [
            entry for entry in _origin()._real_profile_cdp_cache.values()
            if isinstance(entry, dict) and entry.get("copy_root") == copy_root
        ]
        for entry in entries:
            _origin()._real_profile_cdp_cache.pop(entry.get("key"), None)

    with _origin()._cleanup_lock:
        task_sessions = []
        for task_id, info in list(_origin()._active_sessions.items()):
            ...
            if features.get("real_profile") and features.get("real_profile_copy_root") == copy_root:
                ...
                _origin()._active_sessions.pop(task_id, None)
                _origin()._session_last_activity.pop(task_id, None)
    ...
    try:
        ...
        _origin().logger.debug("real-profile cleanup-on-consent-off failed: %s", e)
...
def _launch_real_profile_chrome(real_binary: str, copy_dir: str, *, scope_key: str = "",
                                copy_root: str = "") -> Tuple[Optional[int], Optional[str]]:
    ...
    _bt._real_profile_chrome_procs.append((scope_key, copy_root, chrome_proc))
    ...
    while time.monotonic() < deadline:
        ...
        if chrome_proc.poll() is not None:
            _terminate_real_profile_chrome(scope_key)
...
def _attach_agent_browser_to_real_profile(port: int, copy_dir: str,
                                           session_name: Optional[str] = None) -> Tuple[Optional[str], Optional[str]]:
    ...
    session_name = session_name or _bt._REAL_PROFILE_SESSION
...
def _real_profile_cdp() -> tuple:
    ...
    from hermes_cli.browser_connect import chromium_executable, snapshot_real_profile
    scope, scope_error = _real_profile_scope()
    if scope_error:
        return None, scope_error

    with _bt._real_profile_cdp_lock:
        cached_entry = _bt._real_profile_cdp_cache.get(scope["key"])
        ...
        if existing and _cdp_http_ready(existing) and _cdp_on_data_dir(existing, copy_dir):
            _bt._real_profile_cdp_cache[scope["key"]] = {**scope, "cdp": existing}
        ...
        if surviving:
            cdp, err = _attach_agent_browser_to_real_profile(int(surviving.rsplit(":", 1)[1]), copy_dir, session_name)
            if not cdp:
                return None, err
            _bt._real_profile_cdp_cache[scope["key"]] = {**scope, "cdp": cdp}
        ...
        port, err = _launch_real_profile_chrome(real_binary, copy_dir,
                                                 scope_key=scope["key"], copy_root=scope["copy_root"])
        if port is None:
            return None, err
