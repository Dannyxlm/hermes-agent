...
def _publish_tool_snapshot(
    ...
    staged_engine_names: set, staged_memory_names: set, content_aware: bool, prefix_registered: Optional[set]) -> Optional[set]:
    ...
    with _agent_tools_lock:
        ...
        # Ownership can change even if the public schema names stay identical.
...
def refresh_agent_mcp_tools(
    ...
    staged_memory_names, staged_engine_names = _reinject_post_build_tools(agent, new_defs, new_names)
        ...
        if get_mem_schemas is not None:
            ...
            if memory_provider_tools_enabled(
                ...
                for schema in get_mem_schemas():
                    if _add(schema):
                        staged_memory_names.add(schema["name"])
    except Exception:
