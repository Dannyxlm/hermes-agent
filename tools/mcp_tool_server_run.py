...
class MCPServerRunMixin:
    ...
    async def _park(self, revival_reason: str) -> bool:
        ...
        self._reconnect_event.clear()
        if await self._wait_for_reconnect_or_shutdown(timeout=self._parked_retry_interval) == "shutdown":
            return True
    ...
    async def _prepare_run(self, config: dict) -> bool:
        ...
        self._config = config
        self._parked_retry_interval = _core._resolve_parked_retry_interval(config)
        ...
        if self._teardown_race and not self._session_proven:
            ...
            if self._reconnect_retries > _core._MAX_RECONNECT_RETRIES:
                logger.warning(
                    ...
                    "exhausted), parking; will self-probe every %gs until it recovers (state: degraded → parked)",
                    self.name, _core._MAX_RECONNECT_RETRIES, self._parked_retry_interval)
    ...
    async def _on_transport_error(self, exc: Exception, budget: "_RetryBudget") -> bool:
        ...
        if self._reconnect_retries > _core._MAX_RECONNECT_RETRIES:
            logger.warning(
                "MCP server '%s' failed after %d reconnection attempts, parking; will self-probe every %gs "
                "until it recovers (state: degraded → parked): %s: %s",
                self.name, _core._MAX_RECONNECT_RETRIES, self._parked_retry_interval, type(root).__name__, root)
    ...
    async def _on_permanent_error(self, root: BaseException, budget: "_RetryBudget") -> bool:
        ...
        logger.warning(
            "MCP server '%s' hit a permanent error, parking without retries; will self-probe every %gs "
            "(state: connected → parked): %s: %s", self.name, self._parked_retry_interval, type(root).__name__, root)
