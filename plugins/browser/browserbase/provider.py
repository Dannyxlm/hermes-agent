...
Optional feature knobs::
    ...
    browser:
      browserbase:
        proxies: false             # config.yaml; overrides legacy env knob
        session_metadata: true     # opt in to bounded Browserbase metadata
...
def _browserbase_settings() -> Dict[str, Any]:
    """Return non-secret Browserbase behavior settings from config.yaml."""
    try:
        from hermes_cli.config import read_raw_config

        # Raw config preserves whether the user explicitly set a value. The
        # merged defaults would otherwise mask the legacy env fallback.
        config = read_raw_config()
        browser = config.get("browser") if isinstance(config, dict) else None
        settings = browser.get("browserbase") if isinstance(browser, dict) else None
        return settings if isinstance(settings, dict) else {}
    except Exception as exc:  # noqa: BLE001 - configuration is best-effort
        logger.debug("Could not load browser.browserbase config: %s", exc)
        return {}
...
def _setting_bool(
    ...
    *,
    ...
    """Resolve a boolean setting with config-first legacy env fallback."""
    if key in settings:
        return is_truthy_value(settings[key], default=default)

    if legacy_env and legacy_env in os.environ:
        return is_truthy_value(os.environ[legacy_env], default=default)
    return default
...
class BrowserbaseBrowserProvider(BrowserProvider):
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        settings = _browserbase_settings()

        # Non-secret behavior belongs in config.yaml. Keep the existing env
        # knobs as compatibility fallbacks for older installations.
        enable_proxies = _setting_bool(
            settings,
            "proxies",
            legacy_env="BROWSERBASE_PROXIES",
            default=True,
        )
        enable_session_metadata = _setting_bool(
            settings,
            "session_metadata",
            default=False,
        )
        ...
        if enable_session_metadata:
            # Browserbase metadata is queryable in its dashboard/API and must
            # stay under 512 JSON characters. The task id is generated by
            # Hermes (not copied from page content); cap it defensively anyway.
            session_config["userMetadata"] = {
                "source": "hermes",
                "provider": "browserbase",
                "task_id": str(task_id)[:160],
                "proxies": str(enable_proxies).lower(),
            }
        ...
        try:
            ...
            if response.status_code == 402:
                ...
                if response.status_code == 402 and enable_proxies:
                    ...
                    metadata = session_config.get("userMetadata")
                    if isinstance(metadata, dict):
                        metadata["proxies"] = "false"
                    response = requests.post(
                        f"{config['base_url']}/v1/sessions",
                        headers=headers,
                        json=session_config,
