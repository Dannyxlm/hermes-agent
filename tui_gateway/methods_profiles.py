...
def _configure_cfg_sections(profile_dir, params, applied) -> dict | None:
...
def _(rid, params: dict) -> dict:
    ...
    if any(isinstance(params.get(k), list) for k in ("disabled_skills", "enabled_toolsets", "enabled_mcp_servers")):
        mcp_credentials_required = _configure_cfg_sections(profile_dir, params, applied)
    ...
    return _ok(rid, {"ok": all(applied.values()) if applied else True, "applied": applied,
                     **({"credentials_required": mcp_credentials_required}
                        if mcp_credentials_required is not None else {}),
