...
def _pull_updates(
    ...
    merge_ref = target_ref if target_ref is not None else f"origin/{branch}"
    managed_target = _managed_publication_target(git_cmd, branch)
    if managed_target:
        merge_ref = managed_target
    ...
    try:
        try:
            ...
            if managed_target:
                if _git_run(git_cmd, ["merge", "--ff-only", managed_target]).returncode != 0:
                    print("✗ Managed Desktop update stopped: the checkout cannot fast-forward to Dannyxlm/hermes-agent@main.")
                    print("  No files or refs were reset.")
                    sys.exit(1)
                if (_capture_head_sha(git_cmd, _m().PROJECT_ROOT) or "").lower() != managed_target:
                    print("✗ Managed Desktop update stopped: checkout differs from the approved publication.")
                    sys.exit(1)
        ...
        if sync_upstream and not managed_target:
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not getattr(args, "branch", None) and os.environ.get("HERMES_MANAGED_PUBLICATION_UPDATE") != "1":
    ...
    try:
        ...
        if commit_count == 0:
            if managed_target and (_capture_head_sha(git_cmd, _m().PROJECT_ROOT) or "").lower() != managed_target:
                print("✗ Managed Desktop update stopped: checkout differs from the approved publication.")
                sys.exit(1)
