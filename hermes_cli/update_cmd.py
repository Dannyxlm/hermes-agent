...
def _managed_publication_target(git_cmd, branch):
    """Validate the exact paired publication before checkout changes."""
    if os.environ.get("HERMES_MANAGED_PUBLICATION_UPDATE") != "1":
        return None
    if os.environ.get("HERMES_MANAGED_PUBLICATION_BRANCH", branch) != branch:
        print("✗ Managed Desktop update stopped: the publication branch changed.")
        sys.exit(1)
    target = os.environ.get("HERMES_MANAGED_PUBLICATION_TARGET_SHA", "").lower()
    if not re.fullmatch(r"[0-9a-f]{40}", target):
        print("✗ Managed Desktop update stopped: the approved target is invalid.")
        sys.exit(1)
    fetched = _git_run(git_cmd, ["rev-parse", "--verify", f"origin/{branch}^{{commit}}"])
    if fetched.returncode != 0 or fetched.stdout.strip().lower() != target:
        print("✗ Managed Desktop update stopped: the publication moved after the paired release check.")
        sys.exit(1)
    return target
...
def _pull_updates(
    ...
    try:
        ...
        if _git_run(git_cmd, ["merge", "--ff-only", update_target or f"origin/{branch}"]).returncode != 0:
            if os.environ.get("HERMES_MANAGED_PUBLICATION_UPDATE") == "1":
                print("✗ Managed Desktop update stopped: the checkout cannot fast-forward to Dannyxlm/hermes-agent@main.")
                print("  Reinstall the managed checkout; no files or refs were reset.")
                sys.exit(1)
            _reconcile_diverged_checkout(git_cmd, branch, pre_pull_sha)
        if update_target and (_capture_head_sha(git_cmd, _m().PROJECT_ROOT) or "").lower() != update_target:
            print("✗ Managed Desktop update stopped: the checkout did not land on the approved publication.")
...
def _prepare_checkout_for_update(
    ...
    switch_branch, _windows_gateway_resume, update_target=None):
    ...
    target = update_target or f"origin/{branch}"
    result = _git_run(git_cmd, ["rev-list", f"HEAD..{target}", "--count"], check=True)
    commit_count = int(result.stdout.strip())

    apply_is_shallow = _is_shallow_checkout(git_cmd)
    if commit_count > 0 and apply_is_shallow:
        from hermes_cli.banner import _github_compare_behind
        counted = _github_compare_behind(*_tip_shas(git_cmd, target))
    ...
    if commit_count == 0 and is_fork and branch == "main" and os.environ.get("HERMES_MANAGED_PUBLICATION_UPDATE") != "1":
...
def _finish_pulled_update(
    ...
    if is_fork and branch == "main" and os.environ.get("HERMES_MANAGED_PUBLICATION_UPDATE") != "1":
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        update_target = _managed_publication_target(git_cmd, branch)
        ...
        _plan = _prepare_checkout_for_update(
            ...
            _windows_gateway_resume=_windows_gateway_resume, update_target=update_target)
        commit_count = _plan.commit_count

        if commit_count == 0:
            if update_target and (_capture_head_sha(git_cmd, _m().PROJECT_ROOT) or "").lower() != update_target:
                print("✗ Managed Desktop update stopped: the checkout is not at the approved publication.")
                sys.exit(1)
        ...
        pre_pull_sha = _pull_updates(
            ...
            keep_stash=opts.keep_stash, update_target=update_target)
