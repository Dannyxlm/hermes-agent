...
async def get_healthz():
    ...
    Rich status intentionally performs filesystem, process, config, and
    session-store work.  A reconnect probe only needs to know that the HTTP
    event loop is responsive, so keep this endpoint free of those dependencies.
    """
    return {"ok": True}
...
def _read_persisted_usage_analytics(
    ...
    if _usage_analytics_db_identity(db_path) != identity:
        # The path changed while the cache file was being read. Never return
        # an aggregate associated with the previous database inode.
        return None
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            fd = -1  # ``handle`` owns the descriptor from this point onward.
            with handle:
                json.dump(cached, handle, separators=(",", ":"))
                handle.flush()
                os.fsync(handle.fileno())
            # Deliberately use os.replace directly: unlike the shared config
            # writer, this must replace a hostile final symlink rather than
            # following it to an arbitrary target.
...
def _compute_usage_analytics_cached(
    ...
    """Compute usage off the event loop and briefly cache immutable output.

    ``cache_bucket`` intentionally participates only in the cache key.  The
    database path isolates profiles and test homes; the time bucket bounds
    staleness without adding a cache-invalidation write path to SessionDB.
    """
    del cache_bucket
    ...
    # Existing stores are analytics-only and should never take a schema/write
    # lock.  A brand-new Desktop install has no state.db yet, so allow the
    # first request to initialise the empty schema before subsequent reads use
...
def _submit_usage_analytics(
    ...
    The dedicated one-worker executor keeps SQLite/JSON scans from saturating
    the shared server executor.  Concurrent requests for the same cache key
    share one future, while a small pending-key cap prevents an authenticated
    client from building an unbounded queue of distinct historical scans.
    """
    # Cache expiry must not create a second in-flight scan for the same
    # logical query.  The worker still receives the current bucket for the LRU
    # result key, but pending work coalesces independently across rollover.
    key = (db_path, days)
...
async def get_usage_analytics(days: int = 30, profile: Optional[str] = None):
    ...
    try:
        future = _submit_usage_analytics(
            str(db_path),
            safe_days,
            cache_bucket,
        )
        ...
        raise
    if persisted is not None:
        # Return the last aggregate immediately after a restart while the
        # single-flight worker refreshes it off the event loop.
        return persisted
