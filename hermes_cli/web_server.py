...
def _request_managed_candidate() -> Dict[str, Any]:
    source = _read_managed_update_source()
...
def _configure_auth_gate(
    ...
    if allow_public and host in _LOOPBACK_HOST_VALUES and app.state.trusted_public_hosts:
        _log.warning("Loopback dashboard delegates browser authentication to its declared proxy; "
                     "Host/Origin and session-token checks remain active.")
