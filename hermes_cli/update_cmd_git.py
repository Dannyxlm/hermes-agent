...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    def _eol_only():
        ...
        _probe_run("update-index", "--really-refresh", "-q", check=False)
    ...
    with suppress(Exception):
        ...
        if eol_only:
            ...
            _probe_run("checkout", "--force", "--pathspec-from-file=-", "--pathspec-file-nul", "--",
                       input="\0".join(sorted(eol_only)) + "\0", check=False)
