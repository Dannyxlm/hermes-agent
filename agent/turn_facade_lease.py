...
class DurableTurnLease:
    ...
    def __init__(self, agent, db, session_id: str, holder: str, *, borrowed: bool = False) -> None:
        self.agent = agent
        self.db = db
    ...
    def start(self) -> None:
        if self.borrowed:
            return  # the caller owns renewal and liveness
        with self._lock:
    ...
    def release(self) -> None:
        ...
        if self.borrowed:
            return  # caller still owns the lease and may flush after this turn
        agent = self.agent
        try:
...
def admit_durable_turn_lease(
    ...
    if db is None or not session_id or getattr(agent, "_persist_disabled", False):
        if session_turn_lease_holder:
            raise RuntimeError("Pre-acquired session turn lease cannot be used for this turn")
        return admission
    exists = _durable_session_exists(db, session_id)
    if not session_turn_lease_holder and (
        not exists or not callable(getattr(type(db), "acquire_session_turn_lease", None))
    ):
        return admission
    if exists:
    ...
    if session_turn_lease_holder:
        refresh = getattr(db, "refresh_session_turn_lease", None)
        admitted = callable(refresh) and refresh(session_id, holder, ttl_seconds=LEASE_TTL_SECONDS)
    else:
    ...
    lease = DurableTurnLease(agent, db, session_id, holder, borrowed=bool(session_turn_lease_holder))
    agent._active_session_turn_lease_holder = holder
    agent._active_session_turn_lease_ttl_seconds = LEASE_TTL_SECONDS
    try:
        if exists and not (session_turn_lease_holder and borrowed_session_snapshot_authoritative):
            if waited:
            ...
            if latest_session_id:
                tip_getter = getattr(type(db), "get_compression_tip", None)
                tip = tip_getter(db, session_id) if callable(tip_getter) else session_id
                if tip and str(tip) != session_id and str(agent.session_id) != str(tip):
                    if agent.adopt_live_compression_tip(expected_tip_id=str(tip)) is None:
                        raise RuntimeError("Could not safely adopt the live compression tip")
