...
def claude_code_credentials_path() -> Path:
    ...
    Honors the Claude CLI's ``CLAUDE_CONFIG_DIR`` semantics while retaining the
    managed ``HERMES_CLAUDE_CODE_CONFIG_DIR`` override used by CloudSeed.
    """
    config_dir = (os.environ.get("HERMES_CLAUDE_CODE_CONFIG_DIR", "").strip()
                  or os.environ.get("CLAUDE_CONFIG_DIR", "").strip())
...
def _read_claude_code_credentials_from_file() -> Optional[Dict[str, Any]]:
    ...
    data = _load_json_if_exists(cred_path, "Claude Code credentials file")
    record = _claude_oauth_record(data, "claude_code_credentials_file") if data is not None else None
    if record is not None:
        record["credentialsPath"] = str(cred_path)
    return record
