...
def claude_code_credentials_path() -> Path:
    ...
    Long-running managed services may have a different ``HOME`` from the
    interactive Claude Code login. Honor the Hermes-only override first, then
    Claude Code's own override, so every profile still converges on one shared
    credential store.
    """
    config_dir = (
