...
class TestRealProfileCdpLaunch:
    ...
    def test_reattaches_to_surviving_chrome_instead_of_overlaying_its_profile(self, tmp_path, live_browser_id):
        ...
        with patch.object(bt_cloud, "_use_real_profile", return_value=True), \
            ...
            scope, scope_error = bt_real_profile._real_profile_scope()
            assert scope_error is None
            cdp, err = bt_real_profile._real_profile_cdp()
        if live_browser_id == "/devtools/browser/x":
            assert (cdp, err) == ("http://127.0.0.1:41000", None)
            attach.assert_called_once_with(41000, str(tmp_path), scope["session_name"])
            assert bt._real_profile_cdp_cache[scope["key"]]["cdp"] == cdp
...
class TestConsentConfigRead:
    ...
    def test_scope_changes_with_hermes_home_and_pinned_identity(self, tmp_path):
        ...
        with patch.object(bc, "detect_default_chromium", return_value="chrome"), \
            ...
            first, err = bt_real_profile._real_profile_scope()
        ...
        with patch.object(bc, "detect_default_chromium", return_value="chrome"), \
            ...
            second, err = bt_real_profile._real_profile_scope()
    ...
    def test_cached_task_is_not_reused_after_consent_revocation(self, tmp_path):
        ...
        with patch.object(bt, "get_hermes_home", return_value=tmp_path), \
             patch.object(bt_cloud, "_use_real_profile", return_value=False), \
             patch.object(bt_lifecycle, "_start_browser_cleanup_thread"), \
             patch.object(bt_real_profile, "_agent_browser_close_session"), \
             patch.object(bt_cloud, "_get_cloud_provider", return_value=None), \
             patch.object(bt_session, "_create_local_session", return_value=replacement), \
             patch("hermes_cli.browser_connect.cleanup_real_profile_snapshots"):
            result = bt_session._get_session_info("revoked-task")
...
class TestBrowserExecLocalArg:
    ...
    def test_consent_off_is_inert(self):
        ...
        with patch.object(bu, "_real_profile_consented", return_value=False), \
             patch("tools.browser_tool_real_profile._revoke_real_profile_for_current_home") as revoke:
            err = bu._resolve_real_profile_cdp(env, force_local=True)
