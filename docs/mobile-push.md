...
This opt-in adapter projects canonical events into generic APNs alerts and ActivityKit
updates. It never accepts alert text, a topic, approval answers, or a transcript from
...
* `mobile.push.register(scope + {installation_id, connection_id, device_token,
  environment, categories?})`: `{subscription_id, expires_at}`. Both IDs are UUIDs;
...
* `service.register(principal, scope, installation_id=..., connection_id=...,
  ...
  activity_id="", run_id="")` returns the registration receipt.
...
`startedAt` and `updatedAt` use seconds since 2001-01-01. All displayed strings are
generic; server/profile labels, transcript excerpts, commands, and request text are
excluded. Apple acceptance proves provider transport only; actual notification taps
