...
class SessionMessagesMixin:
    ...
    def replace_messages(self, session_id: str, messages: List[Dict[str, Any]], active_only: bool = False,
        ...
        turn_lease_holder: Optional[str] = None, revoke_turn_lease: bool = False) -> None:
        ...
        def _do(conn):
            if turn_lease_holder:
                self._assert_session_turn_lease_holder_on_conn(conn, [session_id], turn_lease_holder)
            if reject_active_turn_lease:
                self._check_transcript_write_guards(
                    conn, session_id, None, turn_lease_holder=turn_lease_holder,
                    reject_active_turn_lease=not bool(turn_lease_holder), reject_active_compression_lock=True)
            ...
            if revoke_turn_lease:
                conversation_id = self._session_turn_lease_key_on_conn(conn, session_id)
                revoked = conn.execute("DELETE FROM session_turn_leases WHERE conversation_id = ? AND holder = ?",
                                       (conversation_id, turn_lease_holder))
                if revoked.rowcount != 1:
                    raise SessionTurnLeaseLostError(f"Session turn lease lost while cancelling {session_id!r}")
    ...
    def archive_and_compact(self, session_id: str, compacted_messages: List[Dict[str, Any]],
        ...
        turn_lease_holder: Optional[str] = None) -> int:
