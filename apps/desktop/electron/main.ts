...
async function applyUpdates(
  opts: { dirtyStrategy?: string; expectedTargetSha?: string; stopSafeBlockers?: boolean } = {}
) {
  if (updateInFlight) {
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  // Packaged CloudSeed builds stay on the stamped publication branch. Source
  // checkouts retain upstream's current-branch healing behavior.
  const publication = IS_PACKAGED ? readEffectiveDesktopUpdateConfig() : null
  let branch = publication?.branch || 'main'
