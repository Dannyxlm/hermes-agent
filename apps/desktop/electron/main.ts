...
async function performMutationTargetCheck(updateRoot) {
  ...
  // Publication safety compares the stamped identity with the local checkout.
  // It needs no upstream graph or network refresh, even before an apply.
  return {
    identityDirty: identity.dirty,
    installedBranch: identity.branch,
    installedRepository: identity.repository,
...
async function checkManagedPublicationUpdates({ force = false }: { force?: boolean } = {}) {
  const updateRoot = resolveUpdateRoot()
  let { branch } = readEffectiveDesktopUpdateConfig()
  const upstreamTracking = force ? await checkOfficialUpstream(updateRoot) : undefined
  ...
  const status = await readSourceUpdate({
 ...
 */
function isManagedPublication(): boolean {
  return IS_PACKAGED && INSTALL_STAMP?.repository?.toLowerCase() === 'dannyxlm/hermes-agent'
}

function resolveCheckoutUpdateStrategy(expectedTargetSha?: string): UpdaterStrategy {
...
ipcMain.handle(
  ...
  async (_event: Electron.IpcMainInvokeEvent, name: unknown): Promise<{ branch: string }> => {
    ...
    const requested: string = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
